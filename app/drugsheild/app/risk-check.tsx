...
import { healthApi, RiskCheckResult } from '@/services/healthApi';
...
export default function RiskCheckScreen() {
  ...
  const handleCheckRisk = async () => {
    ...
    try {
      const userAllergies = formData.allergies.trim().split(',').map(a => a.trim()).filter(Boolean);
      const response = await healthApi.checkDrugRisk(formData.drugName.trim(), userAllergies);
      setResult(response);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {result && (
          <View style={styles.resultSection}>
            <ResultCard
              risk={result.risk_level.charAt(0).toUpperCase() + result.risk_level.slice(1) as any}
              advice={result.recommendations?.join('\n') || 'No specific recommendations available.'}
              animated={true}
            />
